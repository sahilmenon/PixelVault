...
from pixelvault.encoder import HEIGHT, HEIGHT_4K, WIDTH, WIDTH_4K
from pixelvault.vault import VAULT_DECODED, VAULT_ENCODED, ensure_dirs
...
def cmd_encode(args):
    _check_ffmpeg()
    ensure_dirs()
    from pixelvault.encoder import encode_file
...
def _cmd_encode_chunked(args, src: Path, w: int, h: int, kwargs: dict, chunk_bytes: int):
    """Split *src* into chunks, encode each, and write a .pvault manifest."""
    from pixelvault.encoder import encode_file
    ...
    manifest_path = out_dir / (src.stem + ".pvault")
...
def cmd_decode(args):
    ...
    if source.endswith(".pvault") or (os.path.exists(source) and source.endswith(".pvault")):
...
def _cmd_decode_manifest(args, manifest_path: str):
    """Decode a multi-chunk .pvault manifest: download+decode each chunk and reassemble."""
    from pixelvault.decoder import decode_file
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="pixelvault",
        description=(
            "PixelVault Infinite -- encode any file into a video and back.\n\n"
    ...
    p_enc.add_argument(
        ...
        help=(
            ...
            "A .pvault manifest is written next to the output files. "
            "Combine with --upload to store across multiple YouTube videos. "
            "Decode with: python main.py decode <file>.pvault"
        ),
    )
    p_enc.add_argument(
    ...
    p_enc.add_argument("--description", default="Encoded with PixelVault", help="YouTube description.")
    ...
    p_upl.add_argument("--description", default="Encoded with PixelVault")
